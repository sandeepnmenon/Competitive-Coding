...
{
public:
    void findInTrie(int i, int j, vector<vector<char>> &board, Trie *root, int m, int n,
                    vector<vector<bool>> &visited, unordered_set<string> &ans)
    {
        ...
        findInTrie(i - 1, j, board, root, m, n, visited, ans);
        findInTrie(i, j - 1, board, root, m, n, visited, ans);
        findInTrie(i + 1, j, board, root, m, n, visited, ans);
        findInTrie(i, j + 1, board, root, m, n, visited, ans);
        visited[i][j] = false;
    }
    ...
    vector<string> findWords(vector<vector<char>> &board, vector<string> &words)
    {
        ...
        for (int i = 0; i < m; ++i)
        {
            for (int j = 0; j < n; ++j)
            {
                findInTrie(i, j, board, root, m, n, visited, temp_ans);
            }
        }
